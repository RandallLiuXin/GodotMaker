...
3. Optionally set `DASHSCOPE_BASE_URL` to the matching public endpoint or a
   matching business-space endpoint. Defaults are
...
same order, then the prompt. It validates up to 20 MB per image, 240–8000 px
per edge, a 1:8–8:1 ratio, and supported JPEG/JPG/PNG/BMP/WEBP formats.

Wan 2.7 does not accept transparent PNG inputs. This initial integration fails
closed for alpha-bearing references: provide an opaque image rather than
silently flattening it. `wan2.7-image-pro` permits 4K only for text-to-image;
image editing is limited to 2K. The generator maps the shared `size` and
`aspect_ratio` deterministically to Wan's `WIDTH*HEIGHT` parameter and always
