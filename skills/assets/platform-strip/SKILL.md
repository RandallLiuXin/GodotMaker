...
6. Normalize each already-sliced cell independently with `asset_image_finalize.py --resize <cell_width>x<cell_height> --no-origin`. Save each output under `<normalized-dir>/<segment>.png` and each report at `.godotmaker/asset-generation/reports/<asset_id>_<segment>_normalize.json`. Do not recombine or resize the raw sheet between slicing and this per-cell finalization.
