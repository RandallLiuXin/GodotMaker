...
Carry every visual rule stated in `brief` into the prompt as written; it is the visual specification for this strip. `references` is optional and conditions those rules rather than replacing them. With references, validate every path is readable, preserve each declared role, and pass the actual images to the selected provider. Do not replace image attachments with prompt text. If the pinned provider cannot accept the supplied images, return STOP. If a supplied reference contradicts a rule stated in `brief`, return STOP with a blocker naming the reference role, its path, and the rule. Without references, generate from the written brief.
