...
6. Build one final `SpriteFrames` artifact from the resolved actions. Do not publish a per-action SpriteFrames, portrait runtime artifact, PackedScene, character controller, or detached FX as the runtime result. Preserve action reports and build fingerprints as result evidence.
...
8. Only after every applicable L0-L4 level passes, write the final generic result. It must name the one `SpriteFrames` output and retain the resolved request, action reports, and fingerprint as evidence. `/gm-asset` verifies the result and records it directly in `ASSETS.md`; the skill creates no registry record or index.
