...
Format: [Semantic Versioning](https://semver.org/) — MAJOR.MINOR.PATCH[-PRERELEASE]
...
- Added diagnostic logging at `.godotmaker/log_agent_tool_debug.log` that records every phase of `log_agent_tool.py` so the next failure mode is localizable from artifacts.
...
- Rewired Agent prompt/output trace capture to `PreToolUse`/`PostToolUse` because the `SubagentStart` payload has no `prompt` field and silently wrote 0-byte traces.
