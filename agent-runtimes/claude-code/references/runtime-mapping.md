...
| `native_image_generation` | For `asset_image_model: native`, use the active Claude Code image generation path. For `asset_image_model: codex`, invoke Codex through `codex exec`, follow the generated-path claim protocol in `gm-asset/references/providers/codex.md`, and write the authoritative `--out-report` claim result. If unavailable, gate before asset generation. |
