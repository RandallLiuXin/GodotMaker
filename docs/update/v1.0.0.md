...
> project, then run
> `python tools/publish.py --agent <current-agent> --force <target>` to cleanly
> re-initialize the framework for `v1.0.0`. `<current-agent>` must match the
> workspace's existing `claude-code`, `codex`, `opencode`, or `pi` selection.
> This skips old migrations and re-baselines the migration tracker after
> re-deploy. It does not convert project documents such as `GDD.md`, `PLAN.md`,
> or `ASSETS.md`; review or regenerate incompatible documents before continuing
> the pipeline.

## Added

- Added Pi as a supported coding-agent runtime alongside Claude Code, Codex, and OpenCode (#171).
