...
**`DESIGN.md`** — 项目的视觉合同：一段视觉定位描述、九个图像风格维度（媒介、色彩、形状、线条、材质、光影、空间深度、构图层次、细节密度）、UI 视觉语言章节，以及 Do / Don't 列表。由 `/gm-gdd` 写成，并在之后所有图像生成和视觉审查中保持权威。它是纯文本描述，不含截图或技术参数。美术方向跑偏时，改这个文件。
...
**`MEMORY.md`** — 跨 tag 保存的稳定架构决策与项目约束。它不记录任务历史、运行时错误、gotcha、workaround、Reviewer triage 或其他动态学习；Worker 不能写入。
