...
设置 `DASHSCOPE_API_KEY` 和 `DASHSCOPE_REGION`（`beijing` 或 `singapore`）。API Key 与 endpoint 按地域隔离，不能混用。`DASHSCOPE_BASE_URL` 可选；设置后必须使用对应地域的公共 DashScope endpoint 或业务空间专属域名。运行 `python tools/check_env.py` 可检查配置。
