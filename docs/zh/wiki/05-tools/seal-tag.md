...
所有会改写已封存归档的入口——`archive --force`、`index --force`、
`backfill --force`——都会在替换任何文件**之前**先退役已有的 seal：先把该 tag 从父级
索引里移除，再删掉 manifest，最后才开始写。因此写入中途失败留下的归档会明确显示为
「未封存、未收录」，而不是仍带着 `sealed: true`、但哈希已经对不上实际文件的状态。
如果失败的恰好是退役 seal 这一步，则什么都还没被改写，归档保持原样。
