...
canonical 文档时，摘要里不会出现对应条目，而不是留下一个必定 404 的链接。
...
所有会改写归档的入口——`archive`、`index --force`、`backfill --force`——都会在替换
任何文件**之前**先认领该归档：如果它已封存就先把 tag 从父级索引里移除，再把 manifest
换成「进行中」标记，最后才开始写。因此写入中途失败留下的归档会明确显示为
「未封存、未收录」，而不是仍带着 `sealed: true`、但哈希已经对不上实际文件的状态。
如果失败的恰好是认领这一步，则什么都还没被改写，归档保持原样。
...
即使是复制到一半就中断的运行也一样。每个会改动归档的命令在第一次写入之前，都会把
manifest 换成标注 `stage`（`archiving` / `indexing` / `backfilling`）的「进行中」
标记，因此被中断的归档仍然可识别，不会退化成旧归档的形状。`backfill` 只会接续自己
中断的工作，不碰 finalize 的。

归档上已有的 warning——比如一次失败的 E2E 或截图复制——会被带进 backfill 写出的新
manifest，因此重写不会把一个 partial 归档悄悄标成 complete。反复跑 `--force` 会重新
推导出同样的条目，但不会堆积重复项。
