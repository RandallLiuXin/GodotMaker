...
| `runtime` | 所选 coding agent，与 `tools/agent_runtime.detect_agent` 同一套判定：`.godotmaker/config.yaml` 里 `agent:` 优先于 `agent_runtime:`（与两者在文件中的先后无关），别名归一化（`claude` → `claude-code`），再按已发布目录回退，最后默认 `claude-code` |
