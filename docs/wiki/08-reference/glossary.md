...
**Reviewer** — A sub-agent and a set of 8 specialist skills (`physics`, `animation`, `ui`, `tilemap`, `navigation`, `shader`, `audio`, `particles`). Once every task in `PLAN.md` reaches `completed` and the verifier has passed, a reviewer checks the code for Godot-specific pitfalls documented in each skill's `gotchas.md` and `checklist.md`. The main agent triages each finding into ACCEPT (add a new task to `PLAN.md`), REJECT (the finding is wrong), or SKIP (the finding is real but not worth fixing now). Citation is mandatory for critical/major REJECT/SKIP. Raw findings and triage decisions are not persisted in `MEMORY.md`. See also: *Sub-agent*, *Worker*, *Verifier*.
...
**Tag** — One full pass through the pipeline from `/gm-gdd` to `/gm-finalize` produces one **tag** (SemVer-named: `v0.1.0`, `v0.2.0`, …). After `/gm-finalize` archives the tag's working docs to `docs/tags/<Tag>/` and runs `git tag <Tag>` locally, you can start the next tag with another `/gm-gdd`, or stop the project. Each tag delivers one minimal playable unit. See also: *ROADMAP.md*, *Milestone*, *Tag archive*.

**Tag archive** — The sealed `docs/tags/<Tag>/` directory: the tag's frozen working documents, `memory/` subtree and E2E evidence, plus a generated `SUMMARY.md`, a per-tag `README.md`, and an `evidence/manifest.json` recording every archived file's size, SHA-256 and source revision. `docs/tags/README.md` indexes every sealed tag. Read an archive top-down — parent index, then `SUMMARY.md`, then the canonical documents. Once sealed it is immutable; `/gm-finalize` refuses to rewrite it. See also: *Tag*.
...
**Visual QA** / **VQA** — The process of checking a screenshot of the running game against written criteria. `/gm-evaluate` uses the `visual-qa` skill, backed by the configured `vqa_model` (`native`, Gemini, or OpenAI), to judge each scene rule by rule against the project's `DESIGN.md` visual contract and against the scene's own content requirements. Reference images generated by `/gm-asset` are provenance context only — resemblance to one is never the acceptance bar. See also: *Evaluation*, *DESIGN.md*.

**Worker** — A sub-agent that implements one game task: writes the GDScript code, the unit tests, and an end-to-end test, then reports back with a structured report. The report contains execution results and failure evidence, not an automatic Memory/Learning entry; Workers cannot write root `MEMORY.md` or `memory/`. Workers operate in isolated git worktrees. After a worker finishes, a verifier and then a reviewer must both complete before the next task begins. See also: *Sub-agent*, *Verifier*, *Reviewer*, *Worktree*.
