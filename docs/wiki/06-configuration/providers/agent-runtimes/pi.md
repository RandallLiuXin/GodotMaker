...
Pi does not ship MCP or a native `Agent` tool. GodotMaker publishes a narrow project extension at `.pi/extensions/godotmaker-runtime.ts`. It provides the Godot runtime debug loop and runs worker/verifier/reviewer/decomposer roles as actual isolated Pi child processes. Worker-like requests receive git worktrees and return merge-candidate branches. Before delegating, commit `.pi/`, `.godotmaker/config.yaml` and hooks, `tools/`, and `AGENTS.md`; the extension checks every new worktree for these resources and fails closed if any are absent. If the extension, trust, Git HEAD, or configured Godot binary is unavailable, the dependent role stops with a setup error instead of silently continuing without verification. Pi does not provide a hard per-role filesystem permission boundary: reviewer/verifier mutation prohibitions are role contracts, not a sandbox. Pi child processes also inherit their parent environment and ancestor `AGENTS.md`; run them from a clean project environment without workspace credentials.
